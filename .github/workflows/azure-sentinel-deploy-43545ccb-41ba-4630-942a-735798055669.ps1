...
function AttemptDeployMetadata($deploymentName, $resourceGroupName, $templateObject, $templateType, $paramFileType, $containsWorkspaceParam) {
    ...
    $deploymentInfo | Where-Object { $_.TargetResource -ne "" } | ForEach-Object {
        ...
        if ($sentinelContentKinds.Count -gt 0) {
            ...
            $metadataDeploymentName = "md-$deploymentName"
            ...
            While (($currentAttempt -lt $MaxRetries) -and (-not $isSuccess))
            {
                ...
                Try
                {
                    New-AzResourceGroupDeployment -Name $metadataDeploymentName -ResourceGroupName $ResourceGroupName -TemplateFile $metadataFilePath `
                ...
                {
                    $err = $_
                    if (-not (IsRetryable $metadataDeploymentName))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            $retryDelaySeconds = [math]::Pow($secondsBetweenAttempts, $currentAttempt)
                            $retryDelaySeconds = $retryDelaySeconds - 1
                            Write-Host "[Warning] Failed to deploy metadata for $contentKind with parent resource id $resource with error: $err. Retrying in $retryDelaySeconds seconds..."
                            Start-Sleep -Seconds $retryDelaySeconds
                        }
                        else
                        {
                            Write-Host "[Warning] Failed to deploy metadata for $contentKind with parent resource id $resource after $currentAttempt attempts with error: $err"
                        }
                    }
                }
            }
